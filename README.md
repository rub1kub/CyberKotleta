...
Роль выдаётся только одному участнику — лидеру текущей недели.
Если в голосовых каналах есть активные участники, бот выбирает лидера среди них; если войс пуст, бот выбирает онлайн-лидера недели.
Если онлайн-кандидатов нет, роль остаётся у общего недельного лидера.
Если другой участник обгоняет лидера, бот снимает роль со старого царя, выдаёт новому и публично объявляет захват трона.
Синхронизация идёт каждую минуту, а название роли обновляется в формате `🎙 Войс-царь недели — 42мин`, а после часа — `🎙 Войс-царь недели — 1ч`.
Кандидаты берутся из всех текущих участников голосовых каналов, поэтому если лидер вышел из войса, роль переходит следующему активному участнику.
Если участник с большим недельным временем заходит в войс, он забирает титул на ближайшей синхронизации.
Для замены царя по online/offline статусу нужен включённый Presence Intent у Discord-приложения и `DISCORD_PRESENCES_INTENT=true` в `.env`.
Анонсы и `/voice-king` показывают ники без пингов.
Канал для анонсов задаётся через `VOICE_KING_ANNOUNCE_CHANNEL_ID`; если он указан, бот пишет только туда и не fallback-ает в другие каналы.
Если недельной статистики ещё нет, роль снимается и название возвращается к базовому.
Если у нескольких кандидатов одинаковое время, побеждает первый по алфавиту `display_name`.
